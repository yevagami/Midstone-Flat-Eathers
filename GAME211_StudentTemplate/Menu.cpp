...
#pragma region Rendering
	...
	bool Button::RenderBackground(SDL_Renderer* renderer_) {
		SDL_Color renderColour = isHovering ? onHoveringBackgroundColour : backgroundColour;
	...
	bool Button::RenderBorder(SDL_Renderer* renderer_) {
		SDL_Rect paddedRect = {
				rect.x,
				rect.y,
				rect.w + 2,
	...
	bool Button::RenderText(SDL_Renderer* renderer_) {
		SDL_Color renderColour = isHovering ? onHoveringTextColour : textColour;
...
#pragma endregion
	...
	void Button::HandleEvents(const SDL_Event& event_) {
		...
		case SDL_MOUSEMOTION:
			if (isMouseOver(event_.motion.x, event_.motion.y)) { isHovering = true; }
		...
		case SDL_MOUSEBUTTONDOWN:
			if (event_.button.button == SDL_BUTTON_LEFT) {
				if (OnClick && hitbox.collisionClickCheck(event_.button.x, event_.button.y)) { OnClick(); } }
			break;
